...
class Settings(BaseSettings):
    ...
    LDAP_SERVER: str | None = Field(
        default=None, description="LDAP server address (e.g., ldap://ldap.example.com)"
    )
    LDAP_PORT: int = Field(
        default=389, description="LDAP server port (389 for LDAP, 636 for LDAPS)"
    )
    LDAP_USE_SSL: bool = Field(
        default=False, description="Use SSL/TLS for LDAP connection"
    )
    LDAP_DOMAIN: str | None = Field(
        default=None,
        description="LDAP domain for UPN authentication (e.g., apac.bosch.com)",
    )
    ...
    LDAP_BASE_DN: str | None = Field(
        default=None,
        description="LDAP base DN for user search (e.g., ou=users,dc=example,dc=com)",
    )
    LDAP_USER_FILTER: str = Field(
        default="(uid={username})",
        description="LDAP search filter for users. Use {username} as placeholder",
    )
    LDAP_ATTR_EMAIL: str = Field(default="mail", description="LDAP attribute for email")
    LDAP_ATTR_FULLNAME: str = Field(
        default="cn", description="LDAP attribute for full name"
    )
    LDAP_TIMEOUT: int = Field(
        default=10, description="LDAP connection timeout in seconds"
    )
